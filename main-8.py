...
app = FastAPI(title="Lumexa v8.8 Enterprise", version="8.8.0")
...
# (direct Google API), or "lovable" (Gemini via the gateway). Defaults to
# whichever key is actually configured so this doesn't silently break
# deployments that only set one — set AI_PROVIDER explicitly to force a choice
# if more than one key happens to be set at once.
...
def execute_cq_script_safely(script: str):
    ...
    if obj is None:
        return None, "Script ran without error but did not assign a shape to the 'result' variable."
...
def home():
    ...
    return {
        "status":"Lumexa v8.8 Enterprise — Vibe Engineering Edition",

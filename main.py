...
app = FastAPI(title="Lumexa v8.18 Enterprise (split architecture)", version="8.18.0",
...
def run_calculix_fem(mesh, mat_key, force_n=1000, force_dir="z"):
    ...
    service, not local computation. Returns the same result shape the old
    in-process version did (or None, meaning "fall back to the analytical
    path", exactly as before). Never raises on a reachability/remote error —
    a down or unconfigured analysis service degrades to the analytical
    fallback instead of failing the whole generation request, same
    fail-open philosophy the old local version had for a missing CalculiX
    binary.
    """
    if not ANALYSIS_SERVICE_URL:
        return None  # Not configured — caller falls back to analytical FEA.
    ...
    try:
        ...
        return data.get("fem_result")
    except Exception:
        # Analysis service unreachable, timed out, or errored — degrade to
        # the analytical fallback rather than fail the whole request. A
        # silent fallback here matches the old behavior when CALCULIX was
        # False; the caller (run_analysis_v8) already handles a None return
        # by using the analytical FEA path instead.
        return None
...
# is a documented product feature, not an undocumented moving target.
# Llama 4 Scout gets full quota + the highest free-tier token throughput
# (30,000 TPM); Llama 4 Maverick only gets HALF quota on Groq's free tier —
# Scout is the better default for that reason alone.
GROQ_API_KEY = os.environ.get("GROQ_API_KEY", "")
GROQ_API_URL = "https://api.groq.com/openai/v1/chat/completions"
GROQ_MODEL = os.environ.get("GROQ_MODEL", "meta-llama/llama-4-scout-17b-16e-instruct")
# Llama 4 Scout is natively multimodal in general, but confirm vision support
# specifically on Groq's hosted version before relying on it for image-to-params —
# override GROQ_VISION_MODEL if it doesn't behave as expected there.
...
def home():
    ...
    return {
        "status":"Lumexa v8.18 Enterprise (split architecture) — Vibe Engineering Edition",

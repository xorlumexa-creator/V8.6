...
- Words like "flange", "leg", "vertical/horizontal face", "bent bracket", "angle bracket",
  or "folded sheet metal" describe faces that are NOT coplanar with each other — one must
  be rotated or translated out of the base plane so it has genuine height in the bend
  direction, not just a different in-plane orientation. Example of the WRONG way (two
  flat boxes unioned into an L-shaped outline, both at the same Z, neither actually
  standing up) vs the RIGHT way (build one leg, then .rotate() a copy of it 90° about
  the shared edge so it truly rises out of the base plane before unioning). If a load
  is described as applied "to the vertical flange", that flange must exist as a real
  out-of-plane feature or the load has nothing physical to act on.
"""
...
def rule_engine_v8(mesh,wt_,ctx,mat_key,holes,sharp,fea,part_desc=""):
    ...
    if part_desc:
        pd=part_desc.lower()
        fold_words=("vertical flange","vertical leg","vertical wall","vertical face",
        ...
        if any(w in pd for w in fold_words) and se[1]>0 and se[0]/se[1]<0.3:
            add("R15","HIGH",f"Prompt implies a bent/folded flange but the part is flat "
                f"(smallest dim {se[0]:.1f}mm vs {se[1]:.1f}mm — no out-of-plane feature)",
                "Rebuild with a genuine rotate()/translate() so the described flange/leg "
                "actually rises out of the base plane — a flat coplanar outline is not "
                "a bent bracket","Engineering judgment")
